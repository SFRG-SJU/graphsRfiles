...
k = 2*m #we will be looking for isometric cycles of length k=2m 
...
for (i in V(g1)) {for (j in V(g1)) {
    if ((a*m<= Di[i, j])&(Di[i, j]<=m)) {vertGk<-c(vertGk,c(i,j))}
  }
}


vertGkm<-matrix(vertGk,ncol=2,byrow=TRUE)
vertGkm #the vertices of Gk
...
for (u in V(Gk)){
pu=c(vertGkm[u,]) #pair (x,y) corresponding to vertex u in Gk
ru=which(vertGkm[1:nGk, 1]==pu[2] & vertGkm[1:nGk,2]==pu[1]) #vertex ru in Gk
dtr<- c(dtr,DiGk[u,ru])}

dtr #all distances in Gk from u(x,y) to ru=(y,x)
dtrm=which(dtr==m) #the vertices in Gk with distance to the reverse =m
...
tra=vertGkm[asp$res[[1]],] # a cycle of length 2m

tra

for(i in 1:m) {for(j in (i+1):(m+1)){}}


vertGkm[asp$res[[2]],] # another cycle of length 2m

vertGkm[asp$res[[3]],] # another cycle of length 2m

vertGkm[asp$res[[4]],] # another cycle of length 2m

vertGkm[asp$res[[18]],] # another cycle of length 2m
